...
class AnalysisOrchestratorAgent:
    ...
    def __init__(self):
        ...
        # Carregar schemas e instruções
        base_path = Path(__file__).parent.parent.parent.parent
        schemas_path = base_path / "schemas"
        instrucoes_path = base_path / "instrucoes"
        
        # Carregar report_orders schema
        with open(schemas_path / "report_orders_schema.json", 'r', encoding='utf-8') as f:
            self.report_orders_schema = json.load(f)
        
        # Carregar report_orders instruções
        with open(instrucoes_path / "report_orders_instrucoes.json", 'r', encoding='utf-8') as f:
            self.report_orders_instrucoes = json.load(f)
        
        # Carregar funções proibidas
        with open(instrucoes_path / "funcoes_proibidas_instrucoes.json", 'r', encoding='utf-8') as f:
            self.funcoes_proibidas = json.load(f)
...
📊 SCHEMA DA TABELA:
{json.dumps(self.report_orders_schema, indent=2, ensure_ascii=False)}

📋 INSTRUÇÕES ATHENA:
{json.dumps(self.report_orders_instrucoes, indent=2, ensure_ascii=False)}

🚫 FUNÇÕES PROIBIDAS:
{json.dumps(self.funcoes_proibidas, indent=2, ensure_ascii=False)}

📐 REGRAS DO ROLES.JSON:
